...
def draw_scene(ax, args, Rc, Rt, raw_coach, F_coach, raw_wheels, raw_track,
    ...
    # Coach (grey)
    plot_faces(ax, V_coach, F_coach, facecolor=args.coach_color, edgecolor='#666666', alpha=1.0, lw=0.9)
...
def main():
    ...
    # Colors
    ap.add_argument("--coach_color", type=str, default="#add8e6", help="Coach fill color (light blue)")
    ...
    ax.set_title(f"Coach + Wheels + Track — (order={args.order})", fontsize=10)
    ax.grid(False)
